...
class StepTest {
    ...
    inner class ResultTest {
        ...
        fun `완전히 동일한 단어일 경우 전부 CORRECT`() {
            ...
            assertThat(Step.create(answer, word).code).isEqualTo(Array(5){ Step.Result.CORRECT }.toList())
        }
    ...
    fun `Result가 전부 CORRECT가 아닐 경우에는 틀림(isCorrect가 false)`() {
        assertThat(Step(listOf(Step.Result.CORRECT, Step.Result.MISMATCH, Step.Result.WRONG)).isCorrect).isFalse()
    }
