...
- [x] 게임 시작 메시지를 출력한다.
  - `WORDLE을 6번 만에 맞춰 보세요.\n시도의 결과는 타일의 색 변화로 나타납니다.`
- [x] 타일 결과를 출력한다.
  - [x] 플레이어가 입력한 답안 단어와 비교한 답안 결과에 따라 각 글자의 일치 상태에 맞게 타일을 출력한다.
    - `🟩` 초록색: 오늘의 단어와 비교하여 답안 단어의 글자가 '완전 일치'인 경우
    - `🟨` 노란색: 오늘의 단어와 비교하여 답안 단어의 글자가 '부분 일치'인 경우
    - `⬜️` 회색: 오늘의 단어와 비교하여 답안 단어의 글자가 '불일치'인 경우
  - [x] 지금까지 입력한 답안 단어에 따른 단어 결과 목록을 출력한다.
- [x] 다시 시도 할 때, 다시 시도의 메시지와 그 이유를 함께 출력한다.
- [x] 게임의 결과에 따라 메시지를 출력한다
  - [x] 성공 메시지를 출력한다. `{시도한 횟수} / {최대 시도 가능 횟수}`도 함께 출력한다.
  - [x] 실패 메시지를 출력한다. `{오늘의 단어}`를 함께 출력한다.
...
- [x] 유효한 답안 단어를 최대 6번 입력받아 게임을 한다.
  ...
  - [x] 단어를 입력받아 답안 단어를 만든다.
    - 유효한 답안 단어를 입력 하였을 때,
      - [x] 게임 검증 로직을 수행한다.
      - [x] 타일 결과 목록을 출력한다.
      - [x] 시행 가능 횟수를 1회 차감한다.
    - 유효하지 않은 답안 단어를 입력 하였을 때, 
      - [x] 답안 단어 입력을 다시 시도해야 한다. 다시 시도 하는 이유를 출력한다.
- [x] 단어 결과 목록과 대응하는 타일 결과 목록을 출력한다.
  - [x] 답안 단어 1번 입력하는 경우
  - [x] 답안 단어를 6번 입력하고 게임에 실패하는 경우
- [x] 게임이 종료 된다. 게임의 성공과 실패 여부를 판별한다.
  - 게임에 성공 하였을 때
    - [x] 해당 답안 단어의 단어 결과가 모두 '완전 일치 글자'일 때이다.
      - 관련 메시지 출력
  - 게임에 실패 하였을 때
    - [x] 게임에 성공하지 않고, 시도횟수가 0회가 될 때이다.
